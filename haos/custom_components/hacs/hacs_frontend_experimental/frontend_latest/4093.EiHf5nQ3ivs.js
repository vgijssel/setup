...
export const id = 4093;
export const ids = [4093];
export const modules = {
  64093: (i, t, s) => {
    s.r(t), s.d(t, { FlowLayout: () => _, flow: () => r });
    class e {
      constructor(i) {
        (this._map = new Map()),
          (this._roundAverageSize = !1),
          (this.totalSize = 0),
          !0 === (null == i ? void 0 : i.roundAverageSize) &&
    ...
    var h = s(15723);
    const r = (i) => Object.assign({ type: _ }, i);
    function a(i) {
    ...
    class l {
      constructor() {
        (this._childSizeCache = new e()),
          (this._marginSizeCache = new e()),
      ...
      update(i, t) {
        ...
        Object.keys(i).forEach((e) => {
          const r = Number(e);
          this._metricsCache.set(r, i[r]),
            this._childSizeCache.set(r, i[r][(0, h.qF)(t)]),
            s.add(r),
            s.add(r + 1);
        });
        for (const i of s) {
          ...
          const s =
              (null === (e = this._metricsCache.get(i)) || void 0 === e
                ...
                : e[a(t)]) || 0,
            h =
              (null === (r = this._metricsCache.get(i - 1)) || void 0 === r
                ? void 0
                : r[o(t)]) || 0;
          this._marginSizeCache.set(i, n(s, h));
        }
      }
      ...
      getLeadingMarginValue(i, t) {
        var s;
        return (
          (null === (s = this._metricsCache.get(i)) || void 0 === s
            ? void 0
            : s[a(t)]) || 0
        );
      }
    ...
    class _ extends h.IE {
      constructor() {
        super(...arguments),
          (this._itemSize = { width: 100, height: 100 }),
          (this._physicalItems = new Map()),
          (this._newPhysicalItems = new Map()),
          (this._metricsCache = new l()),
      ...
      _getPhysicalItem(i) {
        var t;
        return null !== (t = this._newPhysicalItems.get(i)) && void 0 !== t
          ? t
      ...
      _getAverageSize() {
        return (
          this._metricsCache.averageChildSize || this._itemSize[this._sizeDim]
        );
      }
      _estimatePosition(i) {
        ...
        if (i < this._first) {
          ...
          return (
            ...
            (t.getMarginSize(this._first - 1) || t.averageMarginSize) -
      ...
      _getPosition(i) {
        var t;
        const s = this._getPhysicalItem(i),
          { averageMarginSize: e } = this._metricsCache;
        return 0 === i
          ? null !== (t = this._metricsCache.getMarginSize(0)) && void 0 !== t
            ? t
            : e
          : s
          ? s.pos
      ...
      _getAnchor(i, t) {
        if (0 === this._physicalItems.size) return this._calculateAnchor(i, t);
        ...
        let r = this._firstVisible - 1,
          a = -1 / 0;
        for (; a < i; ) {
          a =
            this._getPhysicalItem(++r).pos + this._metricsCache.getChildSize(r);
        }
        return r;
      }
      _getActiveItems() {
      ...
      _getItems() {
        ...
        if (((this._stable = !0), null !== this.pin)) {
          const { index: i } = this.pin;
          (this._anchorIdx = i), (this._anchorPos = this._getPosition(i));
        }
        if (
        ...
        void 0 === r && ((this._stable = !1), (r = this._getAverageSize()));
        const a =
            null !== (i = this._metricsCache.getMarginSize(this._anchorIdx)) &&
            void 0 !== i
              ? i
              : this._metricsCache.averageMarginSize,
          o =
            null !==
              (t = this._metricsCache.getMarginSize(this._anchorIdx + 1)) &&
            void 0 !== t
              ? t
              : this._metricsCache.averageMarginSize;
        0 === this._anchorIdx && (this._anchorPos = a),
          this._anchorIdx === this.items.length - 1 &&
            (this._anchorPos = this._scrollSize - o - r);
        let n = 0;
        for (
          this._anchorPos + r + o < e && (n = e - (this._anchorPos + r + o)),
            this._anchorPos - a > h && (n = h - (this._anchorPos - a)),
            n &&
              ((this._scrollPosition -= n),
              (e -= n),
              (h -= n),
              (this._scrollError += n)),
            s.set(this._anchorIdx, { pos: this._anchorPos, size: r }),
            this._first = this._last = this._anchorIdx,
            this._physicalMin = this._anchorPos - a,
            this._physicalMax = this._anchorPos + r + o;
          this._physicalMin > e && this._first > 0;

        ) {
          let i = this._getSize(--this._first);
          void 0 === i && ((this._stable = !1), (i = this._getAverageSize()));
          let t = this._metricsCache.getMarginSize(this._first);
          void 0 === t &&
            ((this._stable = !1), (t = this._metricsCache.averageMarginSize)),
            (this._physicalMin -= i);
          const e = this._physicalMin;
          if (
            (s.set(this._first, { pos: e, size: i }),
            (this._physicalMin -= t),
            !1 === this._stable && !1 === this._estimate)
          )
            break;
        }
        for (; this._physicalMax < h && this._last < this.items.length - 1; ) {
          let i = this._getSize(++this._last);
          void 0 === i && ((this._stable = !1), (i = this._getAverageSize()));
          let t = this._metricsCache.getMarginSize(this._last);
          void 0 === t &&
            ((this._stable = !1), (t = this._metricsCache.averageMarginSize));
          const e = this._physicalMax;
          if (
            (s.set(this._last, { pos: e, size: i }),
            (this._physicalMax += i + t),
    ...
    class r {
      ...
      constructor(i, t) {
        (this._latestCoords = { left: 0, top: 0 }),
          ...
          Promise.resolve().then(
            () => (this.config = t || this._getDefaultConfig())
          );
      }
      set config(i) {
      ...
      set direction(i) {
        (i = "horizontal" === i ? i : "vertical") !== this._direction &&
          ...
          (this._secondarySizeDim = "horizontal" === i ? "height" : "width"),
          (this._positionDim = "horizontal" === i ? "left" : "top"),
          (this._secondaryPositionDim = "horizontal" === i ? "top" : "left"),
          this._triggerReflow());
      }
      get viewportSize() {
      ...
      set viewportScroll(i) {
        ...
        Math.abs(t - this._scrollPosition) >= 1 && this._checkThresholds();
      }
      reflowIfNeeded(i = !1) {
        (i || this._pendingReflow) &&
      ...
      _clampScrollPosition(i) {
        return Math.max(
          ...
          Math.min(i, this.totalScrollSize[e(this.direction)] - this._viewDim1)
        );
      }
      unpin() {
      ...
      _calculateScrollIntoViewPosition(i) {
        const { block: t } = i,
          s = Math.min(this.items.length, Math.max(0, i.index)),
          e = this._getItemPosition(s)[this._positionDim];
        let h = e;
        if ("start" !== t) {
          const i = this._getItemSize(s)[this._sizeDim];
          if ("center" === t) h = e - 0.5 * this._viewDim1 + 0.5 * i;
          else {
            const s = e - this._viewDim1 + i;
            if ("end" === t) h = s;
            else {
              const i = this._scrollPosition;
              h = Math.abs(i - e) < Math.abs(i - s) ? e : s;
            }
          }
        }
        return (
      ...
      _checkThresholds() {
        if ((0 === this._viewDim1 && this._num > 0) || null !== this._pin)
          ...
          const i = Math.max(0, this._scrollPosition - this._overhang),
            t = Math.min(
              this._scrollSize,
      ...
      _updateVisibleIndices(i) {
        ...
        for (
          ...
          s--;
        (t === this._firstVisible && s === this._lastVisible) ||
