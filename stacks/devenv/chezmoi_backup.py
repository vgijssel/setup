...
def get_managed_files():
    ...
    managed_files = [line.strip() for line in output.strip().split("\n") if line.strip()]
...
def get_state_files():
    ...
    try:
        ...
        logger.error("Failed to parse chezmoi state output as JSON. Check the format of the output.")
        return []
...
def get_unmanaged_files():
    ...
    managed_output = run_chezmoi_command(["managed", "--include=files,symlinks", "--path-style=absolute"])
    if not managed_output.strip():
        logger.warning("No managed files found")
        return []
    
    # Split the output into lines and filter out empty lines
    managed_files = [line.strip() for line in managed_output.strip().split("\n") if line.strip()]
...
def reconcile_missing_files():
    ...
    managed_files_output = run_chezmoi_command(["managed", "--include=files,symlinks", "--path-style=absolute"])
    managed_files = set([line.strip() for line in managed_files_output.strip().split("\n") if line.strip()])
        ...
        for file_path, info in entry_state.items():
            if info.get("type") == "file" and file_path not in managed_files:
                ...
                if file_path.startswith(HOME):
                    relative_path = file_path[len(HOME) + 1:]  # +1 to remove the leading '/'
                else:
                ...
                if os.path.exists(unmanaged_path) and os.path.isfile(unmanaged_path):
                    ...
                    logger.info(f"Restored file from unmanaged: {unmanaged_path} to {file_path}")
                else:
                ...
                try:
                    run_chezmoi_command(["state", "delete", "--bucket", "entryState", "--key", file_path])
                    logger.info(f"Removed {file_path} from chezmoi state")
                except Exception as e:
                    logger.error(f"Failed to remove {file_path} from chezmoi state: {e}")
