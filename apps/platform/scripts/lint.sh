#!/usr/bin/env bash
# Render every platform component and validate the manifests with kubeconform.
# CRD instances (ClusterIssuer, ExternalSecret, ...) have no upstream schema, so
# missing schemas are ignored rather than failing the lint.
set -euo pipefail
