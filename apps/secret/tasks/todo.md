...
- [x] **T2.3** `policy-crossplane.yaml` **AND** `policy-admin.yaml` — now Crossplane MRs (reverses the earlier self-init-only decision, per user request that ALL self-init policies be duplicated in Crossplane). self-init still SEEDS both (bootstrap + break-glass independence); Crossplane OWNS/reconciles them thereafter. Safe because self-init is fire-once (no ongoing fight) and OpenBao stores policy text verbatim (no perpetual drift). Verified live: both `SYNCED/READY=True` and stable; Crossplane did not lock itself out reconciling its own policy; break-glass still works. deletionPolicy: Orphan. **NOTE:** the `crossplane` + `admin` *roles* remain self-init-only (not yet mirrored as MRs).
