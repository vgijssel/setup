...
resource "cloudflare_record" "omada_public" {
  ...
  proxied = false
  ttl     = 300
  comment = "network: Omada device inform (managed by OpenTofu)"
}
...
resource "cloudflare_record" "unifi_public" {
  ...
  proxied = false
  ttl     = 300
  comment = "network: UniFi device inform (managed by OpenTofu)"
}
...
resource "cloudflare_record" "omada_private" {
  ...
  proxied = false
  ttl     = 300
  comment = "network: Omada admin UI over Tailscale (managed by OpenTofu)"
}
...
resource "cloudflare_record" "unifi_private" {
  ...
  proxied = false
  ttl     = 300
  comment = "network: UniFi admin UI over Tailscale (managed by OpenTofu)"
}
