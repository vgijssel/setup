...
| Promotion to Hetzner | `moon run network:deploy` (GitHub Actions, `workflow_dispatch`): build **amd64** raw image → compress → **`hcloud-upload-image`** (`--architecture x86`) → snapshot labelled `role=network,arch=amd64` → `tofu apply`, where `hcloud_server.image` is discovered from the newest matching snapshot via `data.hcloud_image` (no hand-copied id) on a `cx` server type. |
