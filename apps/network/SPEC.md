...
| Architecture (prod) | **amd64/x86** — Hetzner `cx` (**decision 2026-07-03 (revised)**, re-reverting the interim arm64 pivot). The uploaded/shipped artifact is `linux/amd64`, and the prod server is **`cx33`** (4 vCPU / 8 GB). Reason the arm64 pivot is no longer needed: the build-time UOS bake only failed because rootless Podman is **broken under QEMU-*emulated* amd64 buildx on the Apple-Silicon host** — not on amd64 itself. Building the amd64 image on a **native x86 GitHub Actions runner** (see `.github/workflows/network-deploy.yaml` + the `build`/`deploy` Moon tasks) runs the bake without any emulation, and amd64 has ample Hetzner capacity (the arm64/`cax` line was exhausted across all EU locations on 2026-07-03). |
| Architecture (local dev) | **arm64 fast loop, unchanged.** The image still builds `linux/arm64` natively and boots **HVF-accelerated** in `qemu-system-aarch64` on the Apple-Silicon Mac (`build-image`/`vm-*`) for the quick edit→boot→persist loop. Since prod is now amd64, the local arm64 boot is a **development proxy**, not the prod-arch gate; the authoritative amd64 validation is the native CI build + the live Hetzner deploy. |
| Omada arch | The TP-Link Omada `.deb` is **`Architecture: all`** (pure Java) — it installs and runs on **both arm64 and amd64** with no force-arch hack (confirmed in the local arm64 VM: admin UI answers HTTP 200 on `:8043` pre/post reboot). So both the local arm64 image and the shipped amd64 image run the **real** Omada, not a dev stand-in. |
...
| Promotion to Hetzner | Two decoupled steps. **Publish (`moon run network:deploy`, GitHub Actions):** build **amd64** raw image → compress → **`hcloud-upload-image`** (`--architecture x86`) → snapshot labelled `role=network,arch=amd64`. It does **not** run tofu. **Provision (`moon run network:apply`, separate/manual):** `tofu apply` on a `cx` server, where `hcloud_server.image` is discovered from the newest matching snapshot via `data.hcloud_image` (no hand-copied id). |
