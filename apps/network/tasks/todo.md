...
- [~] **T6** UniFi OS Server — **PARTIAL (prereqs + first-boot scaffold done; actual install BLOCKED).** Done & baked: `podman 4.9.3` + `slirp4netns` + `uidmap` + `fuse-overlayfs`; `uosserver` user + subuid/subgid baked into `/etc`; official arm64 installer **v5.1.19** staged + **sha256-pinned** (`4e18c1f1…`); `/var/lib/uosserver` bind-mounted onto the **Volume** (rootless graphroot follows HOME); `/var/tmp` bound onto the Volume (installer needs ≥2 GB temp); first-boot `uosserver-install.service` runs `uos-installer --non-interactive` (best-effort). Validated live: podman OK (no baked `storage.conf` — a partial one breaks it; rootless auto-picks overlay+fuse-overlayfs, graphroot on the Volume), installer passes podman + `/var/tmp` + `/var/lib/uosserver` checks.
  **BLOCKER 1 (disk):** the installer hard-requires **≥15 GB free in `/home`**; on Kairos `/home` is on `COS_PERSISTENT` (~9.4 GB) because a 7.7 GB image makes `COS_STATE` eat most of a 40 GB disk. UOS's real data is on the Volume, so this is a conservative check — but fixing it needs a **bigger boot disk (prod cost → SPEC §8 ask-first)** or a `/home`→Volume redirect (SSH-key ordering risk). **Needs an operator decision.**
  **BLOCKER 2 (persistence):** the installer writes `uosserver.service` into Kairos' **ephemeral `/etc`**, so it won't survive reboot — needs a **baked** persistent-start unit (reverse-engineered from a successful install's `systemctl cat uosserver.service`). Pending BLOCKER 1.
  Also: for **prod (amd64)** the amd64 installer URL is needed (the arm64 one was supplied for the local loop).
