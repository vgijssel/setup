...
- [ ] **T2** Tailscale (native apt, pinned): `tailscaled.service` + up oneshot, reusable tagged authkey via cloud-config, stable hostname; persist `/var/lib/tailscale` (bind_mount); joins tailnet on boot, survives reboot-in-VM
