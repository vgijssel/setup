...
- [~] **T6** UniFi OS Server — **build-time install works; runtime load/run pending.** Pivoted to installing UOS **at Docker build time** (operator's call — solves 3 problems at once: `/usr` is writable at build so the installer's `/usr/bin/uosserver` symlink works; `uosserver.service` bakes into `/etc` and survives Kairos' ephemeral `/etc`; the 834 MB installer never ships). Mechanism (all committed, build **green**):
  ...
  **RUNTIME WIRING — in progress (build green; UOS image loads; container start blocked on a home-path mismatch).** Done: `uosserver-load.service` (first-boot) loads the tar via `podman pull oci-archive:` (NB `podman load -i` can't auto-detect this archive) with `TMPDIR` on the Volume (Kairos `/var/tmp` is a tiny tmpfs → "no space" otherwise); it correctly loads `docker.io/library/uosserver:02cb044f5ed2` (2.07 GB). `uosserver.service` + `uosserver-load.service` enabled at build; auto-updater masked (pin version). `network-mount-data.sh` binds the graphroot onto the Volume + chowns the (root-owned-at-runtime) uosserver home.
  **REMAINING BLOCKER (home-path mismatch):** the vendor launcher `uosserver-service` runs Podman against **`/home/uosserver`** (`.config/containers/containers.conf` + `.local` graphroot), NOT the `/var/lib/uosserver` I set via `useradd --home-dir`. Kairos DOES preserve baked `/home` content (via its `.state/home.bind`, unlike `/usr/local` which the empty `COS_PERSISTENT` shadows), but `/home/uosserver` comes up **root-owned** → "`.config/containers/storage.conf` permission denied", and the build-time graphroot landed under `/var/lib/uosserver` (wrong place). **Next:** drop the `--home-dir` override so the whole install uses `/home/uosserver`; chown `/home/uosserver` to uosserver at boot; bind `/home/uosserver/.local` → Volume; load the image there. THEN validate the rootless container actually runs on Kairos (pasta net + `--systemd=always` nested systemd), answers **:11443**, and survives reboot. Prod also needs the **amd64** installer URL.
