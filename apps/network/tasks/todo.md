...
- [ ] **T22** Add per-cluster `tagOwners` (`secret-operator`/`secret-k8s`/`network-operator`/`network-k8s`), additive — keep `tag:k8s`/`tag:k8s-operator` *(deps: none)*
- [ ] **T23** Two operator OAuth clients (`kv/tailscale-secret`/`kv/tailscale-network`) + split `operator-oauth` ExternalSecret per cluster (mirrors T5 store split) *(deps: T22)*
