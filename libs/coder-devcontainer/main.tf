...
data "coder_parameter" "repo" {
  ...
  option {
    name        = "vgijssel/setup"
    description = "Setup repository with devcontainer"
    value       = "https://github.com/vgijssel/setup"
  }
  option {
    name        = "Custom"
...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    ...
    # Ensure the workspace is in the correct location
    if [ ! -d "/workspaces/setup" ]; then
      echo "Error: Workspace not found at /workspaces/setup"
      exit 1
    fi
