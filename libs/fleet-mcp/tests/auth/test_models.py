...
import pytest
from fleet_mcp.auth.models import AccessToken, AuthError, AuthErrorCode, AuthRequest
...
class TestAccessToken:
    ...
    def test_access_token_invalid_characters(self):
        ...
        with pytest.raises(
            ValueError, match="Token must be 43 URL-safe base64 characters"
        ):
            AccessToken(
...
class TestAuthError:
    ...
    def test_auth_error_serialization(self):
        ...
        error = AuthError(
            error=AuthErrorCode.MISSING_TOKEN, error_description="Token missing"
        )
...
class TestAuthRequest:
    ...
    def test_auth_request_non_exempted_path(self):
        ...
        request = AuthRequest(
            path="/mcp/list_agents", authorization_header="Bearer token123"
        )
    ...
    def test_auth_request_malformed_header_no_bearer(self):
        ...
        request = AuthRequest(path="/mcp/list_agents", authorization_header="token123")
        assert request.bearer_token is None

    def test_auth_request_malformed_header_wrong_scheme(self):
        """Test AuthRequest handles wrong authentication scheme."""
        request = AuthRequest(
            path="/mcp/list_agents", authorization_header="Basic dXNlcjpwYXNz"
        )
        assert request.bearer_token is None

    def test_auth_request_empty_token(self):
        """Test AuthRequest handles empty token after 'Bearer'."""
        request = AuthRequest(path="/mcp/list_agents", authorization_header="Bearer ")
