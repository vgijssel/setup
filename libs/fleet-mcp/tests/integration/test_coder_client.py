...
# T014: Workspace creation test using VCR.py directly
...
# T015: Workspace listing test using VCR.py directly
async def test_list_workspaces(coder_base_url, coder_token, my_vcr, vcr_cassette_dir):
    """Test listing workspaces via Coder API"""
    client = CoderClient(base_url=coder_base_url, token=coder_token)

    cassette_path = str(vcr_cassette_dir / "test_list_workspaces.yaml")
    with my_vcr.use_cassette(cassette_path):
...
# T016: Workspace deletion test using VCR.py directly
async def test_delete_workspace(coder_base_url, coder_token, my_vcr, vcr_cassette_dir):
    """Test workspace deletion via Coder API"""
    client = CoderClient(base_url=coder_base_url, token=coder_token)

    cassette_path = str(vcr_cassette_dir / "test_delete_workspace.yaml")
    with my_vcr.use_cassette(cassette_path):
        ...
        # NOTE: No more is_recording checks or asyncio.sleep()!
        # The cassette already has all the polling interactions recorded
        for _ in range(45):
...
# Test for get_template_version_rich_parameters using VCR.py directly
async def test_get_template_version_rich_parameters(
    ...
    cassette_path = str(
        vcr_cassette_dir / "test_get_template_version_rich_parameters.yaml"
    )
    with my_vcr.use_cassette(cassette_path):
        ...
        rich_params = await client.get_template_version_rich_parameters(
            active_version_id
        )
        ...
        assert any(
            "system" in name.lower() and "prompt" in name.lower()
