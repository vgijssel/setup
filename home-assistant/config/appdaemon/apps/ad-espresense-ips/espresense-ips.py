...
class Sensor:
    ...
    def draw_sensor(self, ax, history):
        ...
        mintime = (
            min(history, key=lambda x: x[1])[1].timestamp() if len(history) > 0 else 0
        )
        maxtime = (
            max(history, key=lambda x: x[1])[1].timestamp() if len(history) > 0 else 1
        )

        ax.scatter(
            x=self.position[0], y=self.position[1], label=self.name, color=self.color
        )
...
class Device:
    def __init__(self, name, id, color, sensors):
        ...
        for sensor in sensors:
            self.measure_history[sensor.name] = (sensor, collections.deque(maxlen=7))
    ...
    def add_measure_history(self, sensor, distance):
        self.measure_history[sensor.name][1].appendleft((distance, datetime.now()))
    ...
    def get_aggragate_dist(self, sensor):
        history = list(
            filter(
                lambda event: event[1] > (datetime.now() - timedelta(seconds=10)),
                self.measure_history[sensor.name][1],
            )
        )
        if len(history) == 0:
            return None
        ...
        weights = list(map(lambda event: float(weight(event[1].timestamp())), history))
    ...
    def draw_device(self, ax):
        mintime = (
            min(self.position_history, key=lambda x: x[1])[1].timestamp()
            if len(self.position_history) > 0
            else 0
        )
        maxtime = (
            max(self.position_history, key=lambda x: x[1])[1].timestamp()
            if len(self.position_history) > 0
            else 1
        )
        alpha = interp1d([mintime, maxtime], [0.1, 1])
        ...
        for history in self.position_history:
            ...
            ax.scatter(
                x=history[0][0], y=history[0][1], label=self.name, color=newcolor
            )

        for device, history in self.measure_history.values():
            dist = self.get_aggragate_dist(device)
            device.draw_sensor(ax, [(dist, datetime.now())] if dist is not None else [])
...
class Room:
    ...
    def draw_room(self, ax):
        ax.add_patch(
            matplotPolygon(
                [(point[0], point[1]) for point in self.points],
                edgecolor="grey",
                fill=False,
            )
        )
...
class ESPresenseIps(hass.Hass):
    def initialize(self):
        ...
        for sensor in self.args["rooms"]:
            self.subscribe_to_topic(
                f"{self.args.get('rooms_topic', 'espresense/rooms')}/{sensor}",
                self.handle_sensor_data_message,
            )
            self.subscribe_to_topic(
                f"{self.args.get('rooms_topic', 'espresense/rooms')}/{sensor}/status",
                self.handle_sensor_status_message,
            )
            self.subscribe_to_topic(
                f"{self.args.get('rooms_topic', 'espresense/rooms')}/{sensor}/telemetry",
                self.handle_sensor_telemetry_message,
            )

        if "draw_interval" in self.args:
            self.run_every(
                self.gen_image,
                f"now+{self.args['draw_interval_offset']}",
    ...
    def handle_sensor_data_message(self, event_name, data, *args, **kwargs):
        ...
        prev = (
            device.position_history[0][0] if len(device.position_history) > 0 else None
        )
        ...
        room = min(
            [(room.shapelyPoly.distance(point), room) for room in self.rooms],
            key=lambda x: x[0],
        )[1]
        if room:
            ...
            self.mqtt.mqtt_publish(
                f"{self.args.get('ips_topic', 'espresense/ips')}/{device.id}",
                json.dumps(
                    {
                        "name": device.name,
                        "x": x,
            ...
            self.mqtt.mqtt_publish(
                f"{self.args.get('ips_topic', 'espresense/ips')}/{device.id}/state",
                room.name,
            )
...
def process_not_home_devices(esPresenseIps):
    ...
    for device_entry in esPresenseIps.devices.values():
        ...
        if current_time - last_seen > 5000:
            esPresenseIps.mqtt.mqtt_publish(
                f"{esPresenseIps.args.get('ips_topic', 'espresense/ips')}/{device_entry.id}",
                json.dumps(
                    {
                        ...
                        "fixes": 0,
                        "measures": 0,
                        "currentroom": "not_home",
                    }
                ),
            )
            esPresenseIps.mqtt.mqtt_publish(
                f"{esPresenseIps.args.get('ips_topic', 'espresense/ips')}/{device_entry.id}/state",
                "not_home",
            )
...
def position_solve(distances_to_station, stations_coordinates, last):
    ...
    def error(x, c, r):
        return sum(
            [(abs(distance(x, c[i]) - r[i]) / r[i] ** 2) ** 2 for i in range(len(c))]
        )
    ...
    x0 = (
        last
        if last is not None
        else sum([W[i] * stations_coordinates[i] for i in range(l)])
    )
